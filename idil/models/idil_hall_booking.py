...
class HallBooking(models.Model):
    ...
    facility_ids = fields.Many2many(
        "idil.hall.facility", "booking_id", "facility_id", string="Facilities"
    )
    booking_date = fields.Date(
        string="Booking Date", default=fields.Date.today, required=True
    )
    ...
    amount = fields.Float(
        string="Advance Amount / Down Payment", store=True, default=""
    )
    amount_paid = fields.Float(
        string="Amount Paid", default=0.0, compute="_compute_amount_paid", store=True
    )
    ...
    payment_method_id = fields.Many2one(
        "idil.payment.method", string="Payment Method", required=True
    )
    ...
    bank_reff = fields.Char(string="Bank Reference", required=True, tracking=True)
    payment_ids = fields.One2many(
        "idil.hall.booking.payment", "booking_id", string="Payments"
    )
    ...
    extra_service_ids = fields.One2many(
        "idil.hall.extra.service", "booking_id", string="Extra Services"
    )
    ...
    def read(self, fields=None, load="_classic_read"):
        ...
        for record in records:
            if "status" in record and "end_time" in record:
                if (
                    record["status"] not in ["canceled"]
                    and record.get("end_time")
                    and record["end_time"] < current_time
                ):
        ...
        if ids_to_update:
            for record in records:
                if (
                    record.get("remaining_amount", 0) == 0
                ):  # Access remaining_amount correctly
        ...
        for record in self:
            if record.amount == 0:
                record.remaining_amount = (
                    record.total_price
                )  # If amount is zero, remaining is total price
            else:
                record.remaining_amount = (
    ...
    def create(self, vals):
        ...
        if hall.availability == "maintenance":
            ...
            maintenance_schedule = self.env["idil.hall.schedule"].search(
                [("hall_id", "=", hall.id), ("status", "=", "maintenance")],
                order="end_time desc",
                limit=1,
            )
        ...
        if 0 < booking.remaining_amount < booking.total_price:
            ...
            booking.remaining_amount == 0 and booking.total_price == booking.amount_paid
        ):
                ...
                self.env["idil.hall.booking.payment"].create(
                    {
                        ...
                        "payment_reference": "Initial Payment for Booking {}".format(
                            booking.name
                        ),
                    }
                )
        ...
        for booking in self:
            if booking.start_time and booking.end_time:
                delta = fields.Datetime.from_string(
                    booking.end_time
                ) - fields.Datetime.from_string(booking.start_time)
                booking.duration = (
        ...
        for booking in self:
            if booking.price_per_guest == 0:
                if booking.no_of_guest and booking.hall_id:
                    booking.total_price = (
                        booking.no_of_guest * booking.hall_id.price_per_hour
                    )
    ...
    def _onchange_amount(self):
        ...
        if self.status == "booked":
            raise ValidationError(
                "You cannot modify the 'Amount to Pay' field for a booking that is already booked."
            )
    ...
    def write(self, vals):
        ...
        for booking in self:
            old_total_price = (
                booking.total_price
            )  # Store the old total price before the update
            ...
            if "no_of_guest" in vals or "hall_id" in vals:
                ...
                transaction = self.env["idil.transaction_booking"].search(
                    [("reffno", "=", booking.name)], limit=1
                )
                if transaction:
                    # Adjust the transaction lines based on the price difference
                    self._adjust_transaction_lines_on_price_change(
                        transaction, price_difference, new_total_price
                    )
    ...
    def _adjust_transaction_lines_on_price_change(
        self, transaction, price_difference, new_total_price
    ):
        ...
        if income_line:
            ...
            income_line.write({"cr_amount": new_income_amount})
        ...
        if receivable_line:
            ...
            receivable_line.write({"dr_amount": new_receivable_amount})
        else:
                ...
                self.env["idil.transaction_bookingline"].create(
                    {
                        ...
                        "hall_booking_id": self.id,
                        "bank_reff": self.bank_reff,
                        "description": "Receivable for Hall Booking {}".format(
                            booking.name
                        ),
                        "account_number": booking.hall_id.Receivable_account_id.id,
                        "transaction_type": "dr",
    ...
    def generate_confirmation_slip_pdf(self):
        ...
        for record in self:
            if not record.name:
                raise ValidationError(
                    "Booking must be selected to generate the payment slip."
                )
    ...
    def generate_salary_report_pdf(self, booking_id=None, export_type="pdf"):
        ...
        )  # Default condition to avoid syntax errors
        ...
        if export_type == "pdf":
            ...
            payment_table_data = [
                ...
                [
                    "",
                    "Hall Booking Confirmation SLIP VOUCHER",
                    "",
                ...
                [
                    ...
                    "Booking Date",
                    report_data["booking_date"],
                ],
                [
                    "Client Phone",
                    report_data["client_phone"],
                    "Reff No",
                    report_data["booking_name"],
                ],
                [
                    "Hall Name",
                    report_data["hall_name"],
                    "Booking Status",
                    report_data["status"],
                ],
                ...
                [
                    "No of Guests",
                    f"${report_data['no_of_guest']:,.2f}",
                    "Due Amount",
                    f"${report_data['remaining_amount']:,.2f}",
                ],
                [
                    "Price per Guest",
                    f"${report_data['effective_price']:,.2f}",
                    "Event Type",
                    f"{report_data['event_type']}",
                ],
                [
                    "Gross Total",
                    f"${report_data['no_of_guest'] * report_data['effective_price']:,.2f}",
                    "Booking Start Date",
                ...
                TableStyle(
                    [
                        ...
                        (
                            ...
                            (3, 6),
                            colors.red,
                        ),  # Specify the exact cell for "Due Amount"
            ...
            client_name = report_data[
                "client_name"
            ]  # Employee name from the report data

            signature_table = [
                [
                    f"Prepared by (Admin): {hr_name}",
                    "______________________",
                    f"Paid by (Client): {client_name}",
                    "______________________",
                ],
            ]
...
class HallBookingPayment(models.Model):
    ...
    booking_id = fields.Many2one(
        "idil.hall.booking", string="Hall Booking", required=True, ondelete="cascade"
    )
        ...
        debit_line = self.env["idil.transaction_bookingline"].search(
            [
                ...
                (
                    "account_number",
                    "=",
                    self.payment_method_id.account_number.id,
                ),  # Cash account
                ("transaction_type", "=", "dr"),
            ],
            limit=1,
            ...
            self.env["idil.transaction_bookingline"].create(
                {
                    ...
                    "hall_booking_payment_id": payment_id,
                    "hall_booking_id": self.booking_id.id,
                    "description": "Adjusted Cash Payment for Hall Booking {}".format(
                        booking.name
                    ),
                    "account_number": self.payment_method_id.account_number.id,
                    "transaction_type": "dr",
                    ...
                    self.env["idil.transaction_bookingline"].create(
                        {
                            ...
                            "hall_booking_payment_id": payment_id,
                            "hall_booking_id": self.booking_id.id,
                            "description": "Adjusted Receivable for Hall Booking {}".format(
                                booking.name
                            ),
                            "account_number": booking.hall_id.Receivable_account_id.id,
                            "transaction_type": "dr",
        ...
        def _adjust_booking_lines_on_unlink(self, transaction, deleted_amount):
            ...
            if cash_line:
                ...
                if cash_line.dr_amount >= deleted_amount:
                    cash_line.write({"dr_amount": cash_line.dr_amount - deleted_amount})
                else:
...
class HallBookingPaymentWizard(models.TransientModel):
    ...
    booking_id = fields.Many2one(
        "idil.hall.booking", string="Hall Booking", required=True
    )
    payment_method_id = fields.Many2one(
        "idil.payment.method", string="Payment Method", required=True
    )
    ...
    def default_get(self, fields):
        ...
        booking = self.env["idil.hall.booking"].browse(
            self.env.context.get("active_id")
        )
        if booking:
            res["booking_id"] = booking.id
            res["bank_reff"] = booking.bank_reff  # ✅ prefill
        return res
        ...
        payment = self.env["idil.hall.booking.payment"].create(
            {
                ...
                "payment_reference": "Additional Payment for Booking {}".format(
                    self.booking_id.name
                ),
                "bank_reff": self.bank_reff,
            }
        )
...
class ExtraServiceWizard(models.Model):
    ...
    booking_id = fields.Many2one(
        "idil.hall.booking", string="Booking", required=True, readonly=True
    )
    ...
    account_number = fields.Many2one(
        "idil.chart.account", string="Account Number", store=True
    )
    ...
    def action_save_extra_service_amount(self):
        ...
        if not self.account_number:
            raise UserError(
                "Account number must be selected or derived from the payment method."
            )
        ...
        self.booking_id.write(
            {
                "extra_service_amount": self.booking_id.extra_service_amount
                + self.extra_service_amount,
            }
        )
        ...
        for record in self:
            ...
            if record.exists() and record.booking_id.exists():
                ...
                record.booking_id.write(
                    {
                        "extra_service_amount": record.booking_id.extra_service_amount
                        - record.extra_service_amount,
                    }
                )
                ...
                debit_line = self.env["idil.transaction_bookingline"].search(
                    [
                        (
                            ...
                            "=",
                            record.transaction_booking_id.id,
                        ),
                        ("transaction_type", "=", "dr"),
                ...
                credit_line = self.env["idil.transaction_bookingline"].search(
                    [
                        ...
                        (
                            "account_number",
                            "=",
                            record.booking_id.hall_id.extra_income_account_id.id,
                        ),
                    ],
                    limit=1,
                )
