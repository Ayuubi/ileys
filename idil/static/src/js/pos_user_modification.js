...
export class TicketScreen extends Component {
    ...
    const activeCashier = this.pos.get_cashier(); // Get the logged-in cashier object

    if (!activeCashier) {
        console.warn("No cashier logged in."); // Debug: No cashier found
        return []; // Return empty list if no cashier is logged in
    }

    console.log("Active Cashier Name:", activeCashier.name); // Debug the cashier's name

    if (this._state.ui.filter === "SYNCED") {
        // Filter synced orders based on the cashier's name
        return this._state.syncedOrders.toShow.filter(
            (order) => order.cashier && order.cashier.name === activeCashier.name
        );
    }
    ...
    const predicate = (order) => {
        console.log(
            ...
            "Active Cashier Name:",
            activeCashier.name
        ); // Debug to check if names match

        // Filter by cashier's name and apply search and filter checks
        return (
            order.cashier?.name === activeCashier.name &&
            filterCheck(order) &&
            searchCheck(order)
        );
    };

    // Apply the filtering logic to the order list
    const filteredOrders = this._getOrderList().filter(predicate);

    console.log("Filtered Orders:", filteredOrders); // Debug: List of filtered orders
