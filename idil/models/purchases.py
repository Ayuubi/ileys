...
class PurchaseOrderLine(models.Model):
    ...
    def _create_transaction_line(
        ...
        line_values = {
            ...
            "bank_reff": self.order_id.bank_reff,
        }
    ...
    def _update_item_stock(self, quantity, cost_price):
        if self.item_id:
            try:
                ...
                self.item_id.with_context(disable_transaction_booking=True).write(
        ...
        for order in self:
            ...
            if order.create_uid == self.env.user:
                ...
                if not employee or not employee.maker_checker:
                    raise exceptions.UserError(
                        ...
                        ).format(creator=order.create_uid.name)
                    )
            else:
            ...
            for line in order.order_lines:
                ...
                line._create_item_movement(
                    {"quantity": line.quantity, "cost_price": line.cost_price}
                )
            ...
            _logger.info(
                f"Purchase order {order.id} approved and related transactions created."
            )
